...
import { WorkoutResultByWeightMachine } from '@/types'
...
const config = {
  weight: { max: 150, default: 40, step: 5 },
  times: { max: 50, default: 10, step: 1 },
  set: { max: 10, default: 3, step: 1 }
}

type LegExtensionResultType = {
  index: number
  result: {
    weight: number
    times: number
...
export const initResult = (): WorkoutResultByWeightMachine => ({
  weight: config.weight.default,
  times: config.times.default,
  set: config.set.default
})

export default createComponent<LegExtensionResultType, {}>({
